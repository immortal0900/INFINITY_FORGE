...
사람이 직접 붙이는 정상 실행 라벨은 최초의 `forge:ready-to-build`이다. 카드가 만들어진 뒤의 `forge:*` 라벨은 미러가 카드 상태를 기준으로 다시 쓰므로 임의로 바꿔도 다음 주기에 되돌아갈 수 있다.

...
**Goal:** Windows 로컬, EC2, VPS에서 ChatGPT 구독 Codex를 기본 코딩 runtime으로 사용하고, Forge 코딩 Task 또는 `codex` 스킬이 **구조화된 구독 한도 소진**을 확인한 경우에만 같은 작업·workspace·run을 Claude.ai 구독으로 로그인된 Claude Code CLI로 한 번 이어서 실행한다.
...
8. Claude 실행 전 `claude auth status`가 `loggedIn=true`, `authMethod=claude.ai`, `apiProvider=firstParty`인지 확인한다. `subscriptionType`은 공식적으로 안정된 인증 계약이 아니므로 판정에 사용하지 않는다.
...
17. VPS의 Claude.ai first-party 대화형 로그인 전에는 준비 완료로 보고하지 않으며 해당 머신의 runtime 설정을 부분 적용하지 않는다.
...
Cases: clean apply, 재apply 멱등성, Codex binary 누락, App Server handshake 실패, MCP migration post-check 실패, Claude.ai first-party가 아닌 로그인, backup 복원, Linux mode `0600`, 사용자 Codex 설정의 managed block 밖 보존.
...
- [ ] **Step 3: Claude.ai first-party와 Codex ChatGPT readiness 구현**
...
- Claude CLI 버전 `2.1.215`가 Linux에 없을 때 공식 native installer로 서비스 중지 전에 설치한다.
...
기존 local 검증에 Claude.ai first-party/Codex ChatGPT readiness, configure apply/verify, stable runner/skill 설치, persistent user env 설정, Hermes gateway 시작/재시작, active process 검증을 추가한다. user API env 자체를 삭제하지 않는다.
...
Expected: Codex 설치, Claude `2.1.215`, Claude.ai first-party 로그인, Hermes `0.18.2`. auth 출력은 사용자 응답/로그에 개인정보 없이 요약한다.
...
Remote verify 결과가 Hermes `0.18.2`, ChatGPT Codex, Claude.ai first-party 인증, App Server, `hermes-tools`, 두 managed skill, 일곱 patch target, gateway active를 모두 확인한다.
...
### Task 12: VPS Claude.ai 구독 로그인 checkpoint와 적용
...
deploy preflight가 Claude Code `2.1.215` native binary를 설치하고 `claude auth status`를 확인한다.

- [ ] **Step 2: 사용자 대화형 로그인 checkpoint**

로그인되지 않았으면 VPS에서 `claude auth login`을 사용자가 완료한다. Console/API credential이 아니라 Claude.ai 구독 계정을 선택하고 API credits 제안을 거절한다. 완료 전에는 apply를 실행하지 않는다.

- [ ] **Step 3: 배포 재실행과 readiness**

같은 `deploy.ps1`을 재실행해 fast-forward 상태, Claude.ai first-party 인증, App Server, MCP, skill, patch, gateway를 검증한다.
...
| Claude.ai first-party | PASS 필요 | PASS 필요 | PASS 필요 |
...
구현 commit과 test PASS만으로 완료라고 하지 않는다. Windows·EC2·VPS 모두에서 실제 App Server 활성화, Claude.ai first-party readiness, managed MCP/skill, fake quota 전환, 일반 chat 미영향, API credential 비전달을 확인하고 세 환경 승인 표가 모두 PASS일 때 완료다.

...
| `build` | Build | Build만 실행 |
| `build_review` | Build + Review | Build 다음 Review 실행 |
| `build_review_deep_check` | Build + Review + Deep Check | Build, Review, Deep Check 순서로 실행 |
...
| 역할 ID | 책임 |
|---|---|
| `builder` | **Build**: 요구사항 구현과 PR 생성 또는 수정 |
| `reviewer` | **Review**: 구현 내용과 Acceptance Criteria 대조 |
| `deep_checker` | **Deep Check**: 예외 상황 테스트 추가와 최종 품질 확인 |
| `fix` | **Fix**: Review 또는 Deep Check가 찾은 문제 수정 |

Forge가 소유한 화면과 운영 문서에는 **Task flow, Deep Check, Task Flow Worker, current step, Activity Log, confirmed Task, plain result, System Check, State Mismatch Check, pending messages, selected checks**처럼 기능이 바로 드러나는 이름만 사용한다. 내부 식별값은 화면에서 **내용 식별값**, **설정 식별값**, **현재 commit**처럼 대상을 함께 설명한다.
...
```json
{
  ...
  "task_settings_hash": "64자리 SHA-256",
  "pr_url": "https://github.com/OWNER/REPO/pull/N",
  "built_base_commit": "40자리 Git commit",
...
`result`는 `pass|problems_found`다. `problems_found`에는 비어 있지 않은 `fix_notes`가 필요하다. Deep Check가 테스트를 추가해 commit이 바뀌면 `tested_commit`이 최종 검증 대상이다.
...
```text
Build 결과
  + built_base_commit = current PR base commit
  + open, non-draft PR
...
Build 결과
  + built_base_commit = current PR base commit
  → Review가 built_commit 검토
  → Deep Check가 reviewed_commit에서 추가 테스트 실행
  + deep check result pass
  + tested_commit = current PR commit
  → 작업 흐름 완료
```

Review가 `changes_needed`를 반환하거나 Deep Check가 `problems_found`를 반환하면 같은 PR의 새 Fix 카드를 만든다. 수정 뒤에는 이전 Review 또는 Deep Check 결과를 재사용하지 않고 선택한 흐름을 Build부터 다시 실행한다. 제품 결함 수정은 Task당 최대 3회다. 네 번째 반려에서는 새 Fix 카드를 만들지 않고 `forge:failed`로 끝낸다.
...
|---|---|---|---|
| `build` | Build 뒤 사람이 current commit CI를 확인해 병합 | Build + safe files check + 공통 병합 검사 뒤 자동 병합, 그 밖에는 사람 병합 | Build + 공통 병합 검사 뒤 자동 병합 |
| `build_review` | Build + Review 뒤 사람이 current commit CI를 확인해 병합 | Build + Review + safe files check + 공통 병합 검사 뒤 자동 병합, 그 밖에는 사람 병합 | Build + Review + 공통 병합 검사 뒤 자동 병합 |
| `build_review_deep_check` | Build + Review + Deep Check 뒤 사람이 current commit CI를 확인해 병합 | Build + Review + Deep Check + safe files check + 공통 병합 검사 뒤 자동 병합, 그 밖에는 사람 병합 | Build + Review + Deep Check + 공통 병합 검사 뒤 자동 병합 |
...
| `forge:ready-to-merge` | 선택한 흐름 완료. 사람 또는 Merge Worker가 GitHub 병합 조건을 별도 확인 |
...
각 worker 결과는 자신이 확인한 commit을 기록한다. 다음 단계는 현재 PR base/head가 그 결과와 같을 때만 진행한다. 선택한 흐름이 끝난 뒤 Merge Worker가 CI와 GitHub 병합 조건을 별도로 읽는다.

- `build`: `built_base_commit`과 `built_commit`이 현재 PR base/head commit과 각각 같아야 한다.
- `build_review`: `reviewed_commit`이 현재 PR commit이어야 한다.
- `build_review_deep_check`: Deep Check 입력은 `reviewed_commit`, 최종 현재 PR commit은 `tested_commit`이어야 한다.
- 선택한 흐름이 끝난 뒤 새 push가 있으면 완료 증거를 무효로 하고 `build`부터 다시 실행한다.
- 자동 병합은 현재 PR commit에서 required check가 정확히 한 개 존재하고 `success`일 때만 통과한다. Manual에서도 사람이 같은 조건을 확인한다.
...
새 구현은 이전 이름이나 데이터 형식의 별칭을 제공하지 않는다. 현재 `mode`, `task_flow`, `merge_mode`, 네 role ID, `forge-task:*`, `forge-step:*`, 현재 JSON Schema, 공식 Forge 상태 라벨만 읽는다. 다른 field, enum, key prefix, 결과 파일, 상태 라벨은 명시적 형식 오류다.
...
| `forge/ops/task_settings.py` | Task 설정 형식, 설정 식별값, 만료 판단 |
| `forge/ops/task_service.py` | confirmed Task 기록, GitHub 이슈 생성, 활성화 순서 |
| `forge/ops/task_flow.py` | 선택한 flow의 다음 step 결정 |
| `forge/ops/issue_status.py` | current step을 단일 GitHub label로 표시하고 다시 읽어 확인 |
| `forge/ops/safe_files.py` | Safe Files Auto-Merge 고정 파일 규칙 |
| `forge/ops/merge_runtime.py` | 공통 병합 검사, branch 갱신, 예상 base/head 병합 실행 |
| `forge/ops/github.py` | PR, commit, CI, diff, merge 상태 읽기 |
| `forge/ops/github_merge.py` | 예상 base/head commit에 대한 GitHub 쓰기 |
| `forge/ops/hermes.py` | Hermes Task 읽기와 새 카드 생성 명령 |
...
3. 새 release 실패 시 이전 release와 DB 백업을 복원한다.
4. 복원 뒤 이전 Hermes 일반 대화가 정상 동작한다.
5. 복원 과정이 두 release의 worker를 동시에 실행하지 않는다.
...
- 원본 파일 내용 식별값(`before_file_hash`)
- 설치 파일 내용 식별값(`after_file_hash`)
- 원본 파일 사본(`restore_file`)
- 지원 Hermes commit
- 변경 대상의 정확한 상대 경로 목록:
...
설치 전 현재 파일 내용 식별값이 `before_file_hash`와 다르면 중단한다. 설치 뒤 값이 `after_file_hash`와 다르면 서비스를 시작하지 않고 원본 파일을 복원한다.

기능은 같은 release에 넣되 다음 순서로 활성화한다.

1. Merge Worker unit에 `AUTO_MERGE_ENABLED=false`를 둔 상태로 설치한다.
2. 전체 unit, schema, DB, Hermes 입력 연결 System Check를 실행한다.
3. Chat에서 외부 쓰기 0을 확인한다.
4. **Build + Review + Deep Check + Manual** 시험 Task를 실행한다.
5. Safe Files Auto-Merge Task를 환경값 false로 실행해 `disabled` 결과와 파일 판정을 확인한다.
6. 문서 전용 시험 PR에서만 `AUTO_MERGE_ENABLED=true`로 Safe Files Auto-Merge 실제 병합을 확인한다.
7. 별도 시험 PR에서 All Validated PRs Auto-Merge 실제 병합을 확인한다.
8. 시험이 끝나면 환경값을 다시 false로 두고, production 자동 병합 활성화는 별도 운영 승인으로 수행한다.

즉시 중단 설정은 다음과 같다.

- `AUTO_MERGE_ENABLED=false`: 자동 병합 쓰기만 정지하고 Manual은 유지
- `systemctl --user stop forge-stage.timer forge-mirror.timer forge-merge.timer`: 새 카드·상태·병합 자동 실행 정지
- Hermes 입력 연결 복원: 새 Task 접수를 멈추고 기존 Hermes 대화로 복귀

복원은 새 Task 접수와 Merge Worker를 먼저 멈추고 process lock을 획득한 뒤 수행한다. Task 설정 DB와 Hermes DB를 백업하고, 원본 파일 사본의 내용 식별값을 확인해 대상 파일만 복원한다. 이전 service 정의를 복원하고 Hermes Gateway를 재시작한 뒤 일반 대화와 DB 읽기를 확인한다. Task 설정 기록은 삭제하지 않고 읽기 전용 확인 자료로 남긴다.
...
- 설정 또는 내용 식별값 불일치: 카드 생성과 병합 중단
- GitHub 일시 장애: 제한된 읽기 재시도 뒤 사람 도움 대기
- `safe_auto` 규칙 밖 정상 변경: 실패가 아니라 사람 병합
- 자동 병합 권한 만료: 사람 병합
- branch conflict 또는 반복 갱신: 사람 병합
- 형식 오류, 불완전 API 결과, 알 수 없는 상태: CHECK_ERROR와 사람 도움 대기
- 병합 성공: repository, issue, PR, merged commit, 설정 식별값을 Activity Log에 기록
...
- GitHub Actions 안에서 Build, Review, Deep Check LLM 실행

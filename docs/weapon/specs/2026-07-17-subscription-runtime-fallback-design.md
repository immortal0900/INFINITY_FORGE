...
- 상태: 사용자 승인 완료 (2026-07-17), 현재 브랜치 통합 개정 승인 (2026-07-19)
- 작성일: 2026-07-17
- 적용 대상: Windows 로컬, VPS, EC2
- 결제 원칙: ChatGPT/Codex 구독 및 Claude.ai 구독 로그인만 사용
...
Hermes의 `openai-codex` 인증은 사용할 수 있지만, `anthropic` provider를 Claude.ai 구독으로 직접 대체할 수는 없다. Claude Pro·Max·Team·Enterprise 구독은 Claude Code CLI의 공식 `/login` 경로로 사용해야 한다.
...
- Claude Code: Claude.ai 구독 로그인

Claude Code 인증은 `claude auth status`의 다음 세 필드로 판정한다.

- `loggedIn`이 `true`
- `authMethod`가 `claude.ai`
- `apiProvider`가 `firstParty`

`subscriptionType`은 공식 CLI 문서가 안정적인 인증 계약으로 보장하지 않고 현재 설치된
Claude Code `2.1.215`에서도 `null`일 수 있으므로 허용 여부를 결정하는 필드로 사용하지
않는다. 값이 없거나 `null`이어도 위 세 조건이 맞으면 구독 로그인으로 인정한다. 이 필드는
있더라도 로그·영수증에 저장하지 않는다.
...
사용자 시스템의 원본 환경 변수나 secret 파일은 수정·삭제하지 않는다. 자식 프로세스에
전달되는 환경만 정리한다. 로그인 상태 검사에서 위 세 필드로 Claude.ai first-party 구독
경로를 확인하지 못하면 실행을 중단하고 재로그인을 요구한다. `loggedIn`만으로 실행을
허용하거나 Console/API 인증으로 조용히 전환하지 않는다.
...
- API 키가 자식 환경에서 제거됨
- `subscriptionType` 누락·`null` 상태에서도 Claude.ai first-party 로그인을 허용
- Console/API·Bedrock·Vertex·Foundry 인증은 Claude 자동 호출 전에 거절
...
### 14.1 현재 브랜치 통합 개정 (2026-07-19)

`codex/subscription-runtime-fallback`의 커밋된 최종점 `9084dcf`를
`codex/hybrid-task-control`에 전체 병합한다. 원본 worktree의 미커밋 변경과 대상
worktree의 기존 사용자 변경은 병합 대상이 아니며 보존한다.

병합 뒤 `subscriptionType == "max"`를 요구하는 제품 코드·테스트·배포 검사를 이 문서의
세 필드 인증 계약으로 교체한다. 기존 quota 판별, 한 방향 전환, API 자격 증명 제거,
run 영수증 정책은 유지한다.

이 통합의 완료 범위는 코드 병합, 인증 계약 수정, 문서 정합성, fresh 자동화 검증까지다.
실제 Windows·EC2·VPS 배포, persistent 환경변수 쓰기, Hermes gateway 재시작은 별도 배포
승인 전까지 수행하지 않는다.
...
- [Anthropic Claude Code 인증](https://code.claude.com/docs/en/authentication)
- [Anthropic Claude Code CLI 명령](https://code.claude.com/docs/en/cli-usage)

...
| Windows Hermes | Hermes source `540f901`, Infinity Forge plugin 없음 | Windows runtime 설치 경로 필요 |
...
값 자체는 secret이 아니지만 `.env` 내용은 출력하지 않는다. Task outbox DB는 Task settings DB의 표준 파생 경로를 사용한다.

### 6.2 Hermes user-turn 변경

서버와 같은 `install-hermes-change.py`를 사용한다. package version은 다음 두 commit으로 고정한다.

```text
<Forge commit>-<Hermes source commit>
```

live Hermes checkout에서 직접 package를 만들지 않고 Hermes source commit의 clean archive를 임시 디렉터리에 풀어 build한다. install은 기존 installer의 파일 hash, backup, atomic replace, 부분 실패 restore 규칙을 그대로 사용한다.

### 6.3 plugin·profile·skill

plugin 디렉터리는 sibling temporary directory에서 완성하고 기존 디렉터리와 교체한다. 이어서 `hermes plugins enable infinity-forge`를 실행하고 다시 읽어 enabled 상태를 확인한다.
...
- local plugin list에 `infinity-forge`가 enabled로 표시된다.
...
Hermes source SHA가 달라져 새 patch package가 만들어진다. 기존 patch가 새 upstream과 맞지 않으면 package build/install 단계에서 실패하고 서버·Windows 모두 조용히 넘어가지 않는다.

    ...
    if "def _run_prompt_submit(" in source:
        ...
        source = _insert_after_line_in_unique_block(
            ...
            addition=(
                ...
                "    # next model iteration; never retain any preceding receipt data.",
                ...
                "    _forge_owner_host = _forge_steer_context.get(\"owner_host\")",
                "    _forge_working_directory = _forge_steer_context.get(\"working_directory\")",
                "    try:",
                "        _forge_owner_is_canonical = (",
                "            isinstance(_forge_owner_host, str)",
                "            and str(uuid.UUID(_forge_owner_host)) == _forge_owner_host",
                "        )",
                "    except (AttributeError, TypeError, ValueError):",
                "        _forge_owner_is_canonical = False",
                "    _forge_steer_is_bound = (",
                "        _forge_owner_is_canonical",
                "        and isinstance(source_event_id, str)",
                "        and bool(source_event_id.strip())",
                "        and isinstance(text, str)",
                "        and \"working_directory\" in _forge_steer_context",
                "        and (",
                "            _forge_working_directory is None",
                "            or (",
                "                isinstance(_forge_working_directory, str)",
                "                and bool(_forge_working_directory)",
                "            )",
                "        )",
                "    )",
                "    if _forge_steer_is_bound:",
                "        import hashlib as _forge_hashlib",
                "        _forge_payload_digest = _forge_hashlib.sha256(text.encode(\"utf-8\")).hexdigest()",
                "        _forge_directory_bytes = (",
                "            b\"\\x00\"",
                "            if _forge_working_directory is None",
                "            else b\"\\x01\" + _forge_working_directory.encode(\"utf-8\")",
                "        )",
                "        _forge_directory_digest = _forge_hashlib.sha256(_forge_directory_bytes).hexdigest()",
                "        _forge_bound_payload = (",
                "            \"forge-surface-event-payload/v1\\0\"",
                "            f\"{_forge_owner_host}\\0{_forge_directory_digest}\\0{_forge_payload_digest}\"",
                "        ).encode(\"utf-8\")",
                "        _forge_steer_context[\"source_event_id\"] = source_event_id",
                "        _forge_steer_context[\"source_payload\"] = text",
                "        _forge_steer_context[\"source_payload_hash\"] = _forge_hashlib.sha256(_forge_bound_payload).hexdigest()",
                "    else:",
                "        # Missing/merged events have no single authenticated receipt.",
                "        _forge_steer_context = {}",
                "    setattr(agent, \"_infinity_forge_trusted_turn_context\", _forge_steer_context)",
                "    session[\"_infinity_forge_source_event_id\"] = (",
                "        source_event_id if _forge_steer_is_bound else \"\"",
                "    )",
            ),
            max_lines=76,

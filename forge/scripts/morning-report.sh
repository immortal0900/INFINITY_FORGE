...
# 상단 고정: spec 커버리지 → adr·failed·mergeable/리뷰 대기 → 24h 카드 집계
# + MEMEX 일별 진행상태 미러 1건 outbox 적재(D4 미러의 쿼터 절약 구현)
set -u
DB=~/.hermes/kanban.db
SINCE=$(( $(date +%s) - 86400 ))
...
ADR=$(/usr/bin/gh api "repos/immortal0900/INFINITY_FORGE/issues?state=open&labels=forge:adr&per_page=10" \
  --jq '.[] | "#\(.number) \(.title)"' 2>/dev/null | head -5)
...
# MEMEX 일별 진행상태 미러 (outbox 경유, flush가 배달)
...
리뷰 대기 PR: ${PRS:-없음}. ADR 대기: ${ADR:-없음}. 백업: ${BK:-기록 없음}.

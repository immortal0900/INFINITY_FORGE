...
def test_claude_rate_limit_accepts_first_party_auth_with_null_subscription_type() -> None:
...
def test_claude_auth_requires_all_three_first_party_subscription_fields(
...
def test_claude_rate_limit_accepts_missing_subscription_type() -> None:
    auth = valid_claude_auth()
    auth.pop("subscriptionType")
    events = ({"type": "system", "subtype": "api_retry", "error": "rate_limit"},)

    assert classify_claude_stream(events, auth) is ExitClass.SUBSCRIPTION_QUOTA
